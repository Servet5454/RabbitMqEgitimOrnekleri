...
;
        public Worker(ILogger<Worker> logger, RabbitMqClientsService rabbitMqClientsService, IServiceProvider serviceProvider)
        {
            _logger = logger;
        ...
        protected override Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            _channel.BasicConsume(RabbitMqClientsService.queveName,false,consumer);
        ...
        private async Task Consumer_Received(object sender, BasicDeliverEventArgs @event)
        {
            ...
            var baseurl = "https://localhost:7141/api/files";
            using( var htttpClient =new HttpClient())
            {
                var response =await htttpClient.PostAsync($"{baseurl}?fileId={createExcelMessage.FileId}",multipartFormDataContent);
                if(response.IsSuccessStatusCode)
                {
                    _logger.LogInformation($"File() Id:{createExcelMessage.FileId}: was created by succesfully");
